# AHT20/AHT20/AHT21 I2c-based humiditure sensor support
#
...
######################################################################
# Compatible Sensors:
#       AHT20      -    Tested w/ BTT GTR 1.0 MCU on i2c3
...
class AHT20:
    def __init__(self, config):
        ...
        self.i2c = bus.MCU_I2C_from_config(
            config, default_addr=AHT20_I2C_ADDR, default_speed=100000)
        self.report_time = config.getint('aht20_report_time',30,minval=5)
        self.temp = self.min_temp = self.max_temp = self.humidity = 0.
        self.sample_timer = self.reactor.register_timer(self._sample_aht20)
        self.printer.add_object("aht20 " + self.name, self)
    ...
    def _make_measurement(self):
        ...
        try:
            while is_busy:
                ...
                # and issue warning.
                if is_busy and cycles > AHT20_MAX_BUSY_CYCLES:
                    logging.warning("aht20: device reported busy after " +
                        "%d cycles, resetting device"% AHT20_MAX_BUSY_CYCLES)
                    self._reset_device()
                    data = None
                    break
                ...
                self.i2c.i2c_write(AHT20_COMMANDS['MEASURE'])
                ...
                if read is None:
                    logging.warning("aht20: received data from" +
                                    " i2c_read is None")
                    continue
                data = bytearray(read['response'])
                if len(data) < 6:
                    logging.warning("aht20: received bytes less than" +
            ...
            logging.exception("aht20: exception encountered" +
    ...
    def _sample_aht20(self, eventtime):
        ...
        if self.temp < self.min_temp or self.temp > self.max_temp:
            self.printer.invoke_shutdown(
                "AHT20 temperature %0.1f outside range of %0.1f:%.01f"
...
def load_config(config):
    ...
    pheater.add_sensor_factory("AHT20", AHT20)
